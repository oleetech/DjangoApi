...
const ApprovedInfo = () => {
  ...
  const getApprovedApplications = async () => {
    try {
      const response = await axios.get(`${apiUrl}/leave/details/${status}`);
      console.log(response.data.leaves || []);
      setApproveApplication(response.data.leaves);
    } catch (error) {
      console.log([]);
    }
  };
    ...
    <>
      <div className="max-h-96 max-w-96 sm:max-w-full overflow-auto mt-10">
        <table className="min-w-full text-center text-sm font-light">
          ...
          <tbody>
            {approveApplication.map((item, index) => (
              <tr
      ...
      {selectedLeaveID && <Approved id={selectedLeaveID} />}
    </>
  );
};
